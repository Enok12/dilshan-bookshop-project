...
{
	$search_Query = "SELECT a.ID,a.P_ID,a.GRN_ID,b.S_Name,c.P_Name,a.QTY,a.Status FROM grn_details a,supplier b,product c WHERE c.P_ID=a.P_ID AND b.S_ID=a.S_ID AND a.GRN_ID = '".$_SESSION['GRN']."'";
	...
	while ($row1 = mysqli_fetch_array($search_Result)) 
	{
		...
		echo "<td>"; echo $row1["Status"]."</td>";
			echo "<td>"; ?><a href="DeleteSRNrecord.php?ids=<?php echo $row1["ID"];?>&p_id=<?php echo $row1["P_ID"]; ?>&Stock=<?php echo $row1["QTY"];?>"><button type="button">Delete</button> </a><?php echo "</td>";
		echo "</tr>";
	 echo "</div>";
